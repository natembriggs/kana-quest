...
share — did **not** happen: `app.js`/`vocab.js` (`renderVocabWordGlyph`) and
`src/reader.js` (`isTokenFuriganaHidden`) each carry their own independent
reveal-ladder implementation today, exactly the drift risk phase 8 and
`stories-plan.md` §5.7 both warned about. That extraction is the one piece
of this plan still genuinely outstanding.
