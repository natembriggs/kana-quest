...
Status: **in progress** — phases 0-7, 3a and 3b done (see §12's phase table).
...
(§4.5), the word detail screen (with kanji chips into the existing kanji
detail screen, and per-mode study toggles), a Higher tier for the
frequency-fallback data (own unit per theme, browsed as one "Common words 2"
group), and an A level tier (§2.3's own 13 themes, its own "A level" group,
A12 hand-authored the same way Core is) are all live behind the fourth
"Vocabulary" script on the home screen. A13 ("The set text and film")
remains a deliberate stub until a specific text or film is chosen — see
phase 7.

**Stories are no longer "not yet built"** — they shipped as their own
feature; see `stories-plan.md`, which is live. What phase 8 below asked for
on their behalf — extracting `src/furigana.js` as a component both features
share — happened, but only **partially**: commit `6f5394d` (2026-09-04) pulled
just the shared "is this reading hidden" three-way check (known /
exposure-promoted / muted) out of both callers into `isReadingHidden()` in a
new `src/furigana.js`, now imported by both `app.js` and `src/reader.js`. Left
deliberately unmerged — per that commit and `review-followups.md`'s own note
on it, "diverge by design" — are the surrounding aggregation
(`renderVocabWordGlyph`/`vocabHiddenState` in `app.js` still decide per kanji
position and layer the quiz's own partial-furigana-askability override;
`isTokenFuriganaHidden`/`tokenAtLevel` in `src/reader.js` still decide per
whole word) and the two tap-state machines (`vocabRevealLevel` vs
`tokenAtLevel`). So the byte-identical duplicate this section used to point
at is gone, but `app.js`/`vocab.js` and `src/reader.js` still each render
their own reveal ladder — the fuller "one module renders a word into any
container" component §10 point 3 asked for was never built, a narrower target
was chosen instead, and some drift risk remains.
...
| 7 | **A level** — the A-group units from §2.3. Data plus the group labels. | 6 | **Done**, on the same phase-0 fallback as GCSE, with a twist: `build_vocab_data.py` reuses phase 6's exact mechanism (a second keyword-classification pass, `THEME_KEYWORDS_A`/`classify_a()`, over whatever the 'f'/'h' pass didn't already claim from the SAME frequency-ranked candidate list) rather than a different technique, because the concern phase 6 raised — "a frequency cut cannot produce exam-appropriate abstract vocabulary" — cuts the other way for A level's own themes: JMdict's `nf` bands are newspaper-corpus frequency, a poor fit for GCSE's everyday topics but a reasonable proxy for A level's (economy, politics, media, environment, health — what newspapers actually write about). 11 of the 12 non-stub themes cleared `MIN_UNIT_SIZE` (only A10 needed its keyword list widened, iteratively, to clear it — see the build script's own comment). A12 "Writing and arguing" is A level's Core-equivalent (essay connectives and abstract nouns — したがって, 客観的...) and is hand-authored via `A12_ENTRIES`, the same rationale and mechanism as `CORE_ENTRIES`: automated ranking finds the wrong homograph as often as the right one for a set this idiomatic (もっとも's best keb match by raw frequency is 最も "most", not the conjunction this wants). A13 "The set text and film" is the plan's own deliberate stub and is never emitted — it has no keyword list and would always be empty. On the JS side: `vocab.js` grew `unitGroup()`/`compareUnits()` recognition of the "A" group (A12 sorted first within it, the group itself sorted last, after "Common words 2"), and `unitLevelLabel()` returns null for it — A level isn't itself split into two further levels, so there is nothing for that label to say beyond what `unitGroupLabel`'s "A level" already does. Settings' "Vocabulary word lists" card and the Vocabulary screen's own hint were updated to describe it. |
| 8 | **Story hooks** — extract `src/furigana.js` as a standalone component if it hasn't already fallen out of phase 3, and confirm `vocab-lookup.js` answers what stories need. | 3 | **Partially done.** Stories shipped (`stories-plan.md`) with `src/reader.js` implementing its own reveal-ladder logic rather than sharing one with `vocab.js`/`app.js`, but `6f5394d` (2026-09-04) later extracted the one piece of that logic which was genuinely byte-identical — the "is this reading hidden" predicate — into `src/furigana.js`'s `isReadingHidden()`, imported by both `app.js` and `src/reader.js`. Everything around that predicate (per-word vs. per-kanji aggregation, the quiz's partial-furigana-askability override, the two separate tap-state machines) was left unmerged on purpose — see the header above. `vocab-lookup.js` itself is used by stories as intended, but only at build time: `tools/build_story_data.mjs` reads it to resolve a story word to its vocab id (`token.d`) while authoring, so the browser never has to ship or query the index at runtime — `src/reader.js` never imports `vocab-lookup.js` directly, and resolves an already-baked-in `token.d` via `vocabCourseForId()` in `app.js`, the same function non-story vocab lookups use. Two implementations of "kanji with optional ruby that reveals on tap" still exist and can still drift outside the shared predicate, per stories-plan.md §5.7's own prediction — just a narrower shared surface than before. |
