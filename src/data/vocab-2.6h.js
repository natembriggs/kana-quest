// Generated by tools/build_vocab_data.py — do not hand-edit.
// Source: JMdict (c) EDRDG, CC BY-SA 4.0 — see build script for the
// frequency-based word-selection approach (vocab-plan.md §3.5 fallback).
// https://www.edrdg.org/wiki/index.php/JMdict-EDICT_Dictionary_Project
// Example sentences (`ex`): the Tanaka Corpus, as distributed with
// WWWJDIC and maintained by the Tatoeba Project, CC BY 2.0 FR.
// https://www.edrdg.org/wiki/index.php/Tanaka_Corpus

export const VOCAB_ENTRIES = [
  {
    "id": "温度",
    "w": "温度",
    "r": "おんど",
    "en": [
      "temperature"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "おん",
        "オン"
      ],
      [
        1,
        "ど",
        "ド"
      ]
    ],
    "mis": [
      "おんたく",
      "おんたび",
      "ぬくど",
      "おんと",
      "おんた",
      "あたたど"
    ],
    "sp": [
      "保温",
      "態度",
      "年度",
      "緯度",
      "温厚",
      "度数",
      "温存",
      "経度",
      "密度",
      "毎度",
      "忖度",
      "温泉",
      "二度",
      "丁度",
      "屹度",
      "今度"
    ],
    "ex": [
      {
        "j": "物質は温度によって形を変える。",
        "en": "Matter changes its form according to temperature.",
        "r": [
          [0, 1, "ぶっ"],
          [1, 1, "しつ"],
          [3, 1, "おん"],
          [4, 1, "ど"],
          [9, 1, "かたち"],
          [11, 1, "か"]
        ],
        "w": [
          [0, 2, "物質|ぶっしつ"],
          [2, 1, "は@*"],
          [3, 2],
          [5, 4, "に因って"],
          [9, 1, "形|かたち"],
          [10, 1, "を@*"],
          [11, 3]
        ]
      },
      {
        "j": "氷の温度は何度ですか。",
        "en": "What is the temperature of the ice?",
        "r": [
          [0, 1, "こおり"],
          [2, 1, "おん"],
          [3, 1, "ど"],
          [5, 1, "なん"],
          [6, 1, "ど"]
        ],
        "w": [
          [0, 1, "氷|こおり"],
          [2, 2],
          [4, 1, "は@*"],
          [5, 2],
          [7, 3]
        ]
      },
      {
        "j": "朝から温度が下がってきた。",
        "en": "The temperature has fallen since morning.",
        "r": [
          [0, 1, "あさ"],
          [3, 1, "おん"],
          [4, 1, "ど"],
          [6, 1, "さ"]
        ],
        "w": [
          [0, 1, "朝|あさ"],
          [1, 2, "から@*"],
          [3, 2],
          [5, 1, "が#2028930@*"],
          [6, 4, "下がる"],
          [10, 2, "来る|くる"]
        ]
      }
    ]
  },
  {
    "id": "かすみ",
    "w": "かすみ",
    "r": "かすみ",
    "en": [
      "haze (esp. in spring)",
      "mist",
      "dimness (of sight)"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "sn": [
      2,
      1
    ],
    "uk": true
  },
  {
    "id": "泉",
    "w": "泉",
    "r": "いずみ",
    "en": [
      "spring",
      "fountain"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "いずみ",
        "いずみ"
      ]
    ],
    "mis": [
      "せん"
    ],
    "sp": [
      "何",
      "糠",
      "翼",
      "逆",
      "黴",
      "塔",
      "園",
      "塵",
      "筒",
      "量",
      "親",
      "労",
      "米",
      "唇",
      "版",
      "制"
    ],
    "ex": [
      {
        "j": "その男は知識の泉ともいうべき人だった。",
        "en": "The man was a fountain of knowledge.",
        "r": [
          [2, 1, "おとこ"],
          [4, 1, "ち"],
          [5, 1, "しき"],
          [7, 1, "いずみ"],
          [14, 1, "ひと"]
        ],
        "w": [
          [0, 2, "其の|その"],
          [2, 1, "男|おとこ"],
          [3, 1, "は@*"],
          [4, 2],
          [7, 1],
          [8, 2, "とも@2"],
          [10, 2, "言う|いう"],
          [12, 2, "可き"],
          [14, 1, "人|ひと"],
          [15, 3, "だ"]
        ]
      },
      {
        "j": "泉はおよそ１７０フィートの高さまで温水を吹き出す。",
        "en": "The fountain sends hot water to a height of 170 feet.",
        "r": [
          [0, 1, "いずみ"],
          [13, 1, "たか"],
          [17, 1, "おん"],
          [18, 1, "すい"],
          [20, 1, "ふ"],
          [22, 1, "だ"]
        ],
        "w": [
          [0, 1],
          [1, 1, "は@*"],
          [2, 3, "凡そ|およそ"],
          [8, 4, "呎"],
          [13, 2],
          [15, 2, "迄"],
          [17, 2],
          [19, 1, "を@*"],
          [20, 4]
        ]
      },
      {
        "j": "彼らはその泉で喉の渇きをいやした。",
        "en": "They satisfied their thirst at the spring.",
        "r": [
          [0, 1, "かれ"],
          [5, 1, "いずみ"],
          [7, 1, "のど"],
          [9, 1, "かわ"]
        ],
        "w": [
          [0, 2, "彼ら|かれら"],
          [2, 1, "は@*"],
          [3, 2, "其の|その"],
          [5, 1],
          [6, 1, "で#2028980@*"],
          [7, 1, "喉|のど"],
          [9, 2],
          [11, 1, "を@*"],
          [12, 4, "癒す"]
        ]
      }
    ]
  },
  {
    "id": "天気",
    "w": "天気",
    "r": "てんき",
    "en": [
      "weather",
      "fair weather",
      "fine weather"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "sn": [
      1,
      2
    ],
    "ruby": [
      [
        0,
        "てん",
        "テン"
      ],
      [
        1,
        "き",
        "キ"
      ]
    ],
    "mis": [
      "てんいき",
      "あまき",
      "あまつき",
      "でんき",
      "てんぎ",
      "てんけ",
      "あめき"
    ],
    "sp": [
      "香気",
      "天明",
      "短気",
      "天職",
      "天童",
      "気絶",
      "狂気",
      "気密",
      "臭気",
      "外気",
      "一気",
      "天文",
      "色気",
      "正気",
      "気丈",
      "湯気"
    ],
    "ex": [
      {
        "j": "彼は天気のよい日を利用して壁を塗った。",
        "en": "He took advantage of the fine weather to paint the wall.",
        "r": [
          [0, 1, "かれ"],
          [2, 1, "てん"],
          [3, 1, "き"],
          [7, 1, "ひ"],
          [9, 1, "り"],
          [10, 1, "よう"],
          [13, 1, "かべ"],
          [15, 1, "ぬ"]
        ],
        "w": [
          [0, 1, "彼|かれ"],
          [1, 1, "は@*"],
          [2, 2],
          [5, 2, "良い|よい"],
          [7, 1, "日|ひ"],
          [8, 1, "を@*"],
          [9, 2],
          [11, 2, "為る|する"],
          [13, 1, "壁|かべ"],
          [14, 1, "を@*"],
          [15, 3, "塗る|ぬる"]
        ]
      },
      {
        "j": "今日が天気ならいいのに。",
        "en": "I wish it were fine today.",
        "r": [
          [0, 2, "きょう"],
          [3, 1, "てん"],
          [4, 1, "き"]
        ],
        "w": [
          [0, 2, "今日|きょう"],
          [2, 1, "が#2028930@*"],
          [3, 2, "天気@2"],
          [5, 2, "なら@*"],
          [7, 2, "いい#2820690"],
          [9, 2, "のに@*"]
        ]
      },
      {
        "j": "天気がよいから散歩に行きたい気分だ。",
        "en": "It's a nice day and I feel like taking a walk.",
        "r": [
          [0, 1, "てん"],
          [1, 1, "き"],
          [7, 1, "さん"],
          [8, 1, "ぽ"],
          [10, 1, "い"],
          [14, 1, "き"],
          [15, 1, "ぶん"]
        ],
        "w": [
          [0, 2],
          [2, 1, "が#2028930@*"],
          [3, 2, "良い|よい"],
          [5, 2, "から@*"],
          [7, 2],
          [9, 1, "に@*"],
          [10, 2, "行く|いく"],
          [12, 2],
          [14, 2],
          [16, 1]
        ]
      }
    ]
  },
  {
    "id": "冬季",
    "w": "冬季",
    "r": "とうき",
    "en": [
      "winter season",
      "winter"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "とう",
        "トウ"
      ],
      [
        1,
        "き",
        "キ"
      ]
    ],
    "mis": [
      "どうき",
      "とうぎ",
      "ふゆき"
    ],
    "sp": [
      "季語",
      "冬眠",
      "冬山",
      "真冬",
      "季刊",
      "四季",
      "冬場",
      "望郷",
      "道教",
      "重症",
      "使命",
      "皇位",
      "由緒",
      "後世",
      "繁殖",
      "戦前"
    ],
    "ex": [
      {
        "j": "冬季のフロリダの気候はアイオワより穏和だ。",
        "en": "The climate of Florida in the wintertime is milder than that of Iowa.",
        "r": [
          [0, 1, "とう"],
          [1, 1, "き"],
          [8, 1, "き"],
          [9, 1, "こう"]
        ],
        "w": [
          [0, 2],
          [3, 4],
          [8, 2],
          [10, 1, "は@*"],
          [15, 2, "より@5"],
          [17, 2, "温和"],
          [19, 1]
        ]
      }
    ]
  },
  {
    "id": "被災",
    "w": "被災",
    "r": "ひさい",
    "en": [
      "suffering (from a disaster)",
      "being hit (by an earthquake, typhoon, etc.)",
      "falling victim (to)"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "ひ",
        "ヒ"
      ],
      [
        1,
        "さい",
        "サイ"
      ]
    ],
    "mis": [
      "かぶさい",
      "ひざい",
      "びさい",
      "こうむさい",
      "ひわざわ",
      "ぴさい",
      "おおさい"
    ],
    "sp": [
      "災害",
      "法被",
      "戦災",
      "被膜",
      "被服",
      "労災",
      "被覆",
      "人災",
      "火災",
      "災難",
      "被告",
      "開明",
      "三宝",
      "残暑",
      "旅団",
      "板前"
    ],
    "ex": [
      {
        "j": "市は地震の被災者に食料と毛布を支給した。",
        "en": "The city supplied the earthquake victims with food and blankets.",
        "r": [
          [0, 1, "し"],
          [2, 1, "じ"],
          [3, 1, "しん"],
          [5, 1, "ひ"],
          [6, 1, "さい"],
          [7, 1, "しゃ"],
          [9, 1, "しょく"],
          [10, 1, "りょう"],
          [12, 1, "もう"],
          [13, 1, "ふ"],
          [15, 1, "し"],
          [16, 1, "きゅう"]
        ],
        "w": [
          [0, 1, "市|し"],
          [1, 1, "は@*"],
          [2, 2, "地震|じしん"],
          [5, 3],
          [8, 1, "に@*"],
          [9, 2],
          [11, 1, "と@*"],
          [12, 2],
          [14, 1, "を@*"],
          [15, 2],
          [17, 2, "為る|する"]
        ]
      },
      {
        "j": "被災地に救援物資が急送された。",
        "en": "Relief supplies were raced to the disaster area.",
        "r": [
          [0, 1, "ひ"],
          [1, 1, "さい"],
          [2, 1, "ち"],
          [4, 1, "きゅう"],
          [5, 1, "えん"],
          [6, 1, "ぶっ"],
          [7, 1, "し"],
          [9, 1, "きゅう"],
          [10, 1, "そう"]
        ],
        "w": [
          [0, 3],
          [3, 1, "に@*"],
          [4, 4],
          [8, 1, "が#2028930@*"],
          [9, 2],
          [11, 3, "為れる"]
        ]
      },
      {
        "j": "被災者に衣食を供給しなければならない。",
        "en": "We must provide food and clothes for the victims.",
        "r": [
          [0, 1, "ひ"],
          [1, 1, "さい"],
          [2, 1, "しゃ"],
          [4, 1, "い"],
          [5, 1, "しょく"],
          [7, 1, "きょう"],
          [8, 1, "きゅう"]
        ],
        "w": [
          [0, 3],
          [3, 1, "に@*"],
          [4, 2],
          [6, 1, "を@*"],
          [7, 2],
          [9, 1, "為る|する"],
          [10, 8, "ねばならない"]
        ]
      }
    ]
  },
  {
    "id": "温泉",
    "w": "温泉",
    "r": "おんせん",
    "en": [
      "hot spring",
      "onsen",
      "hot spring resort",
      "hot spring spa"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "sn": [
      1,
      3
    ],
    "ruby": [
      [
        0,
        "おん",
        "オン"
      ],
      [
        1,
        "せん",
        "セン"
      ]
    ],
    "mis": [
      "あたたせん",
      "おんいずみ",
      "おんぜん",
      "ぬくせん"
    ],
    "sp": [
      "室温",
      "適温",
      "低温",
      "体温",
      "温床",
      "温熱",
      "温存",
      "温和",
      "温室",
      "温情",
      "保温",
      "高温",
      "温帯",
      "常温",
      "気温",
      "泉水"
    ],
    "ex": [
      {
        "j": "温泉はあなたに効くでしょう。",
        "en": "The hot spring will do you good.",
        "r": [
          [0, 1, "おん"],
          [1, 1, "せん"],
          [7, 1, "き"]
        ],
        "w": [
          [0, 2],
          [2, 1, "は@*"],
          [3, 3, "貴方|あなた"],
          [6, 1, "に@*"],
          [7, 2],
          [9, 4]
        ]
      },
      {
        "j": "ひなびた温泉が好きです。",
        "en": "I like hot springs off the beaten track.",
        "r": [
          [4, 1, "おん"],
          [5, 1, "せん"],
          [7, 1, "す"]
        ],
        "w": [
          [0, 4, "鄙びる"],
          [4, 2],
          [6, 1, "が#2028930@*"],
          [7, 2, "好き|すき"],
          [9, 2]
        ]
      },
      {
        "j": "今、温泉に行くかスキーに行くかもめているんだよ。",
        "en": "We're torn between going to a hot spring and going skiing.",
        "r": [
          [0, 1, "いま"],
          [2, 1, "おん"],
          [3, 1, "せん"],
          [5, 1, "い"],
          [12, 1, "い"]
        ],
        "w": [
          [0, 1, "今|いま"],
          [2, 2],
          [4, 1, "に@*"],
          [5, 2, "行く|いく"],
          [7, 1, "か@*"],
          [8, 3],
          [11, 1, "に@*"],
          [12, 2, "行く|いく"],
          [14, 1, "か@*"],
          [15, 5, "揉める"],
          [20, 2, "のだ"],
          [22, 1]
        ]
      }
    ]
  },
  {
    "id": "温暖",
    "w": "温暖",
    "r": "おんだん",
    "en": [
      "warm",
      "mild",
      "temperate"
    ],
    "pos": "adj",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "おん",
        "オン"
      ],
      [
        1,
        "だん",
        "ダン"
      ]
    ],
    "mis": [
      "あたただん",
      "おんのん",
      "おんあたた",
      "ぬくだん"
    ],
    "sp": [
      "暖炉",
      "暖房",
      "室温",
      "適温",
      "温室",
      "温厚",
      "保温",
      "気温",
      "温存",
      "体温",
      "高温",
      "温床",
      "常温",
      "水温",
      "温帯",
      "寒暖"
    ],
    "ex": [
      {
        "j": "島の天候はとても温暖です。",
        "en": "The island has a mild climate.",
        "r": [
          [0, 1, "しま"],
          [2, 1, "てん"],
          [3, 1, "こう"],
          [8, 1, "おん"],
          [9, 1, "だん"]
        ],
        "w": [
          [0, 1, "島|しま"],
          [2, 2],
          [4, 1, "は@*"],
          [5, 3, "迚も|とても"],
          [8, 2],
          [10, 2]
        ]
      },
      {
        "j": "私は地球の温暖化傾向を心配している。",
        "en": "I'm worried about the global warming trend.",
        "r": [
          [0, 1, "わたし"],
          [2, 1, "ち"],
          [3, 1, "きゅう"],
          [5, 1, "おん"],
          [6, 1, "だん"],
          [7, 1, "か"],
          [8, 1, "けい"],
          [9, 1, "こう"],
          [11, 1, "しん"],
          [12, 1, "ぱい"]
        ],
        "w": [
          [0, 1, "私|わたし"],
          [1, 1, "は@*"],
          [2, 3, "地球"],
          [5, 2],
          [7, 1],
          [8, 2],
          [10, 1, "を@*"],
          [11, 2],
          [13, 4, "為る|する"]
        ]
      },
      {
        "j": "概して、温暖な国の人々は早寝早起きである。",
        "en": "As a rule, the inhabitants of warm countries keep early hours.",
        "r": [
          [0, 1, "がい"],
          [4, 1, "おん"],
          [5, 1, "だん"],
          [7, 1, "くに"],
          [9, 2, "ひとびと"],
          [12, 1, "はや"],
          [13, 1, "ね"],
          [14, 1, "はや"],
          [15, 1, "お"]
        ],
        "w": [
          [0, 3],
          [4, 3, "温暖"],
          [7, 1],
          [9, 2, "人々|ひとびと"],
          [11, 1, "は@*"],
          [12, 5],
          [17, 3]
        ]
      }
    ]
  },
  {
    "id": "気象",
    "w": "気象",
    "r": "きしょう",
    "en": [
      "weather (conditions)",
      "disposition",
      "temperament"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "sn": [
      1,
      2
    ],
    "ruby": [
      [
        0,
        "き",
        "キ"
      ],
      [
        1,
        "しょう",
        "ショウ"
      ]
    ],
    "mis": [
      "ぎしょう",
      "きかたど",
      "きぞう",
      "いきしょう",
      "けしょう",
      "きじょう"
    ],
    "sp": [
      "英気",
      "気勢",
      "印象",
      "気丈",
      "堅気",
      "気配",
      "気品",
      "気長",
      "気骨",
      "陽気",
      "気味",
      "湯気",
      "気筒",
      "気味",
      "湿気",
      "鬼気"
    ],
    "ex": [
      {
        "j": "環境汚染は、異常な気象状況を引き起こしている。",
        "en": "Environmental pollution is causing abnormal weather conditions.",
        "r": [
          [0, 1, "かん"],
          [1, 1, "きょう"],
          [2, 1, "お"],
          [3, 1, "せん"],
          [6, 1, "い"],
          [7, 1, "じょう"],
          [9, 1, "き"],
          [10, 1, "しょう"],
          [11, 1, "じょう"],
          [12, 1, "きょう"],
          [14, 1, "ひ"],
          [16, 1, "お"]
        ],
        "w": [
          [0, 4],
          [4, 1, "は@*"],
          [6, 3, "異常"],
          [9, 2],
          [11, 2],
          [13, 1, "を@*"],
          [14, 8, "引き起こす"]
        ]
      },
      {
        "j": "気象庁は今晩雨が降ると言っている。",
        "en": "The weather bureau says it will rain tonight.",
        "r": [
          [0, 1, "き"],
          [1, 1, "しょう"],
          [2, 1, "ちょう"],
          [4, 1, "こん"],
          [5, 1, "ばん"],
          [6, 1, "あめ"],
          [8, 1, "ふ"],
          [11, 1, "い"]
        ],
        "w": [
          [0, 3],
          [3, 1, "は@*"],
          [4, 2],
          [6, 1],
          [7, 1, "が#2028930@*"],
          [8, 2, "降る|ふる"],
          [10, 1, "と@*"],
          [11, 5, "言う|いう"]
        ]
      },
      {
        "j": "機長は気象訓示を受ける為に気象室へ行く。",
        "en": "The captain will go to the meteorological office to get a weather briefing.",
        "r": [
          [0, 1, "き"],
          [1, 1, "ちょう"],
          [3, 1, "き"],
          [4, 1, "しょう"],
          [5, 1, "くん"],
          [6, 1, "じ"],
          [8, 1, "う"],
          [11, 1, "ため"],
          [13, 1, "き"],
          [14, 1, "しょう"],
          [15, 1, "しつ"],
          [17, 1, "い"]
        ],
        "w": [
          [0, 2],
          [2, 1, "は@*"],
          [3, 2],
          [5, 2],
          [7, 1, "を@*"],
          [8, 3],
          [11, 2],
          [13, 2],
          [15, 1, "室|しつ"],
          [16, 1],
          [17, 2, "行く|いく"]
        ]
      }
    ]
  },
  {
    "id": "今秋",
    "w": "今秋",
    "r": "こんしゅう",
    "en": [
      "this autumn",
      "this fall",
      "autumn of this year"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "こん",
        "コン"
      ],
      [
        1,
        "しゅう",
        "シュウ"
      ]
    ],
    "mis": [
      "こんじゅう",
      "こんあき",
      "いましゅう",
      "こんとき",
      "ごんしゅう",
      "きんしゅう"
    ],
    "sp": [
      "今様",
      "今昔",
      "今後",
      "今時",
      "今期",
      "秋分",
      "千秋",
      "古今",
      "昨今",
      "今般",
      "今晩",
      "今日",
      "只今",
      "中秋",
      "今更",
      "精錬"
    ]
  },
  {
    "id": "昨秋",
    "w": "昨秋",
    "r": "さくしゅう",
    "en": [
      "autumn of last year",
      "fall of last year"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "さく",
        "サク"
      ],
      [
        1,
        "しゅう",
        "シュウ"
      ]
    ],
    "mis": [
      "さくとき",
      "さっしゅう",
      "ざっしゅう",
      "さくじゅう",
      "ざくしゅう",
      "さくあき"
    ],
    "sp": [
      "千秋",
      "中秋",
      "一昨",
      "昨今",
      "昨日",
      "秋分",
      "戸数",
      "他者",
      "成長",
      "軍票",
      "鉄壁",
      "時評",
      "海老",
      "酪農",
      "名曲",
      "透析"
    ],
    "ex": [
      {
        "j": "昨秋は晴天続きだった。",
        "en": "We had a spell of fine weather last autumn.",
        "r": [
          [0, 1, "さく"],
          [1, 1, "しゅう"],
          [3, 1, "せい"],
          [4, 1, "てん"],
          [5, 1, "つづ"]
        ],
        "w": [
          [0, 2],
          [2, 1, "は@*"],
          [3, 2, "晴天|せいてん"],
          [5, 2, "続く"],
          [7, 3, "だ"]
        ]
      }
    ]
  },
  {
    "id": "今夏",
    "w": "今夏",
    "r": "こんか",
    "en": [
      "this summer",
      "next summer",
      "last summer"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "こん",
        "コン"
      ],
      [
        1,
        "か",
        "カ"
      ]
    ],
    "mis": [
      "こんなつ",
      "いまか",
      "こんが",
      "こんげ",
      "ごんか",
      "きんか"
    ],
    "sp": [
      "今様",
      "今晩",
      "今後",
      "今期",
      "今般",
      "今昔",
      "夏場",
      "昨今",
      "古今",
      "今日",
      "夏山",
      "今更",
      "只今",
      "盛夏",
      "今時",
      "山元"
    ],
    "ex": [
      {
        "j": "あの店では今夏物の特売中です。",
        "en": "The store is having a sale on summer goods.",
        "r": [
          [2, 1, "みせ"],
          [5, 1, "こん"],
          [6, 1, "か"],
          [7, 1, "もの"],
          [9, 1, "とく"],
          [10, 1, "ばい"],
          [11, 1, "ちゅう"]
        ],
        "w": [
          [0, 2, "彼の|あの"],
          [2, 1, "店|みせ"],
          [3, 1, "で#2028980@*"],
          [4, 1, "は@*"],
          [5, 2],
          [7, 1, "物|もの"],
          [9, 2],
          [11, 1, "中|ちゅう@4"],
          [12, 2]
        ]
      },
      {
        "j": "今夏の降水量は普通でなかった。",
        "en": "This summer we had an unusual amount of rain.",
        "r": [
          [0, 1, "こん"],
          [1, 1, "か"],
          [3, 1, "こう"],
          [4, 1, "すい"],
          [5, 1, "りょう"],
          [7, 1, "ふ"],
          [8, 1, "つう"]
        ],
        "w": [
          [0, 2],
          [3, 3],
          [6, 1, "は@*"],
          [7, 2, "普通|ふつう"],
          [9, 5, "である"]
        ]
      }
    ]
  },
  {
    "id": "予報",
    "w": "予報",
    "r": "よほう",
    "en": [
      "forecast",
      "prediction"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "よ",
        "ヨ"
      ],
      [
        1,
        "ほう",
        "ホウ"
      ]
    ],
    "mis": [
      "しゃほう",
      "よぼう",
      "あらかじほう",
      "よぽう",
      "よむく"
    ],
    "sp": [
      "予定",
      "報告",
      "吉報",
      "予習",
      "月報",
      "速報",
      "旬報",
      "誤報",
      "予約",
      "報酬",
      "会報",
      "予告",
      "情報",
      "予防",
      "公報",
      "日報"
    ],
    "ex": [
      {
        "j": "今夜の予報は雨である。",
        "en": "Rain is forecast for this evening.",
        "r": [
          [0, 1, "こん"],
          [1, 1, "や"],
          [3, 1, "よ"],
          [4, 1, "ほう"],
          [6, 1, "あめ"]
        ],
        "w": [
          [0, 2],
          [3, 2],
          [5, 1, "は@*"],
          [6, 1],
          [7, 3]
        ]
      },
      {
        "j": "長期予報によると、暖冬だそうだ。",
        "en": "The long-range forecast says we will have a mild winter.",
        "r": [
          [0, 1, "ちょう"],
          [1, 1, "き"],
          [2, 1, "よ"],
          [3, 1, "ほう"],
          [9, 1, "だん"],
          [10, 1, "とう"]
        ],
        "w": [
          [0, 2],
          [2, 2],
          [4, 4, "に依ると"],
          [9, 2],
          [11, 1],
          [12, 3, "そうだ@2"]
        ]
      },
      {
        "j": "予報はさらに降雪があると伝えていた。",
        "en": "The prediction was for more snow.",
        "r": [
          [0, 1, "よ"],
          [1, 1, "ほう"],
          [6, 1, "こう"],
          [7, 1, "せつ"],
          [12, 1, "つた"]
        ],
        "w": [
          [0, 2],
          [2, 1, "は@*"],
          [3, 3, "更に"],
          [6, 2],
          [8, 1, "が#2028930"],
          [9, 2, "有る"],
          [11, 1, "と@*"],
          [12, 5, "伝える"]
        ]
      }
    ]
  },
  {
    "id": "前線",
    "w": "前線",
    "r": "ぜんせん",
    "en": [
      "(weather) front",
      "front line",
      "the front",
      "battlefront"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "sn": [
      1,
      3
    ],
    "ruby": [
      [
        0,
        "ぜん",
        "ゼン"
      ],
      [
        1,
        "せん",
        "セン"
      ]
    ],
    "mis": [
      "ぜんすじ",
      "まえせん",
      "ぜんぜん"
    ],
    "sp": [
      "幹線",
      "前座",
      "前記",
      "食前",
      "前者",
      "脱線",
      "建前",
      "線描",
      "空前",
      "霊前",
      "目線",
      "前期",
      "前編",
      "線路",
      "午前",
      "前方"
    ],
    "ex": [
      {
        "j": "彼らは兵士として前線に行った。",
        "en": "As soldiers they went to the front.",
        "r": [
          [0, 1, "かれ"],
          [3, 1, "へい"],
          [4, 1, "し"],
          [8, 1, "ぜん"],
          [9, 1, "せん"],
          [11, 1, "い"]
        ],
        "w": [
          [0, 2, "彼ら|かれら"],
          [2, 1, "は@*"],
          [3, 2],
          [5, 3],
          [8, 2],
          [10, 1, "に@*"],
          [11, 3, "行く|いく"]
        ]
      },
      {
        "j": "彼は前線へやられた。",
        "en": "He was sent into combat.",
        "r": [
          [0, 1, "かれ"],
          [2, 1, "ぜん"],
          [3, 1, "せん"]
        ],
        "w": [
          [0, 1, "彼|かれ"],
          [1, 1, "は@*"],
          [2, 2],
          [4, 1],
          [5, 4, "遣る"]
        ]
      }
    ]
  },
  {
    "id": "天候",
    "w": "天候",
    "r": "てんこう",
    "en": [
      "weather"
    ],
    "pos": "n",
    "th": "2.6",
    "lv": "h",
    "ruby": [
      [
        0,
        "てん",
        "テン"
      ],
      [
        1,
        "こう",
        "コウ"
      ]
    ],
    "mis": [
      "てんごう",
      "あまこう",
      "あまつこう",
      "てんそうろう",
      "あめこう",
      "でんこう"
    ],
    "sp": [
      "天理",
      "居候",
      "天命",
      "満天",
      "仰天",
      "天日",
      "天国",
      "天女",
      "先天",
      "気候",
      "天才",
      "天文",
      "天皇",
      "症候",
      "天使",
      "天童"
    ],
    "ex": [
      {
        "j": "天候が悪くて私は釣りに行けなかった。",
        "en": "The bad weather prevented me from going fishing.",
        "r": [
          [0, 1, "てん"],
          [1, 1, "こう"],
          [3, 1, "わる"],
          [6, 1, "わたし"],
          [8, 1, "つ"],
          [11, 1, "い"]
        ],
        "w": [
          [0, 2],
          [2, 1, "が#2028930@*"],
          [3, 3, "悪い|わるい"],
          [6, 1, "私|わたし"],
          [7, 1, "は@*"],
          [8, 9, "釣りに行く"]
        ]
      },
      {
        "j": "それはすべて天候に依存している。",
        "en": "It all depends on the weather.",
        "r": [
          [6, 1, "てん"],
          [7, 1, "こう"],
          [9, 1, "い"],
          [10, 1, "ぞん"]
        ],
        "w": [
          [0, 2, "其れ"],
          [2, 1, "は@*"],
          [3, 3, "全て"],
          [6, 2],
          [8, 1, "に@*"],
          [9, 2, "依存|いぞん"],
          [11, 4, "為る|する"]
        ]
      },
      {
        "j": "もし天候がよければ大集会となろう。",
        "en": "The gathering will be large if the weather is good.",
        "r": [
          [2, 1, "てん"],
          [3, 1, "こう"],
          [9, 1, "だい"],
          [10, 1, "しゅう"],
          [11, 1, "かい"]
        ],
        "w": [
          [0, 2, "若し|もし"],
          [2, 2],
          [4, 1, "が#2028930@*"],
          [5, 4, "良い|よい"],
          [9, 1, "大|だい"],
          [10, 2],
          [12, 4, "となる"]
        ]
      }
    ]
  }
];
