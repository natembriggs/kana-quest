// Generated by tools/build_vocab_data.py — do not hand-edit.
// Source: JMdict (c) EDRDG, CC BY-SA 4.0 — see build script for the
// frequency-based word-selection approach (vocab-plan.md §3.5 fallback).
// https://www.edrdg.org/wiki/index.php/JMdict-EDICT_Dictionary_Project
// Example sentences (`ex`): the Tanaka Corpus, as distributed with
// WWWJDIC and maintained by the Tatoeba Project, CC BY 2.0 FR.
// https://www.edrdg.org/wiki/index.php/Tanaka_Corpus

export const VOCAB_ENTRIES = [
  {
    "id": "運賃",
    "w": "運賃",
    "r": "うんちん",
    "en": [
      "(passenger) fare",
      "freight rates",
      "shipping expenses"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "sn": [
      1,
      2
    ],
    "ruby": [
      [
        0,
        "うん",
        "ウン"
      ],
      [
        1,
        "ちん",
        "チン"
      ]
    ],
    "mis": [
      "うんぢん",
      "はこちん"
    ],
    "sp": [
      "運休",
      "家賃",
      "運搬",
      "運命",
      "運勢",
      "機運",
      "運動",
      "開運",
      "命運",
      "運営",
      "運送",
      "不運",
      "運河",
      "運航",
      "運行",
      "海運"
    ],
    "ex": [
      {
        "j": "運賃は片道1ドルです。",
        "en": "The fare is one dollar each way.",
        "r": [
          [0, 1, "うん"],
          [1, 1, "ちん"],
          [3, 1, "かた"],
          [4, 1, "みち"]
        ],
        "w": [
          [0, 2],
          [2, 1, "は@*"],
          [3, 2],
          [6, 2, "弗"],
          [8, 2]
        ]
      },
      {
        "j": "この価格には、運賃は含まれていません。",
        "en": "Delivery is not included in the price.",
        "r": [
          [2, 1, "か"],
          [3, 1, "かく"],
          [7, 1, "うん"],
          [8, 1, "ちん"],
          [10, 1, "ふく"]
        ],
        "w": [
          [0, 2, "此の|この"],
          [2, 2],
          [4, 2, "には@*"],
          [7, 2],
          [9, 1, "は@*"],
          [10, 8, "含む|ふくむ"]
        ]
      },
      {
        "j": "バスや列車に乗るときには運賃を払わなければならない。",
        "en": "When riding a bus or a train, you need to pay the fare.",
        "r": [
          [3, 1, "れっ"],
          [4, 1, "しゃ"],
          [6, 1, "の"],
          [12, 1, "うん"],
          [13, 1, "ちん"],
          [15, 1, "はら"]
        ],
        "w": [
          [0, 2, "バス#1098390@*"],
          [2, 1, "や@*"],
          [3, 2],
          [5, 1, "に@*"],
          [6, 2],
          [8, 2, "時|とき"],
          [10, 2, "には@*"],
          [12, 2],
          [14, 1, "を@*"],
          [15, 2, "払う"],
          [17, 8, "ねばならない"]
        ]
      }
    ]
  },
  {
    "id": "漁船",
    "w": "漁船",
    "r": "ぎょせん",
    "en": [
      "fishing boat"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "ぎょ",
        "ギョ"
      ],
      [
        1,
        "せん",
        "セン"
      ]
    ],
    "mis": [
      "あさせん",
      "ぎょふな",
      "ぎょぜん",
      "ぎょふね",
      "りょうせん"
    ],
    "sp": [
      "帆船",
      "船便",
      "商船",
      "密漁",
      "漁師",
      "本船",
      "造船",
      "豊漁",
      "船室",
      "船底",
      "艦船",
      "船体",
      "船長",
      "乗船",
      "船尾",
      "配船"
    ],
    "ex": [
      {
        "j": "海岸から約１マイル沖に漁船が見えた。",
        "en": "I saw a fishing boat about a mile off the shore.",
        "r": [
          [0, 1, "かい"],
          [1, 1, "がん"],
          [4, 1, "やく"],
          [9, 1, "おき"],
          [11, 1, "ぎょ"],
          [12, 1, "せん"],
          [14, 1, "み"]
        ],
        "w": [
          [0, 2],
          [2, 2, "から@*"],
          [4, 1],
          [6, 3, "哩"],
          [9, 1],
          [10, 1, "に@*"],
          [11, 2],
          [13, 1, "が#2028930@*"],
          [14, 3, "見える|みえる"]
        ]
      },
      {
        "j": "行方不明だった漁船が無事帰港した。",
        "en": "The fishing boat that had been missing returned to its port safely.",
        "r": [
          [0, 1, "ゆ"],
          [1, 1, "くえ"],
          [2, 1, "ふ"],
          [3, 1, "めい"],
          [7, 1, "ぎょ"],
          [8, 1, "せん"],
          [10, 1, "ぶ"],
          [11, 1, "じ"],
          [12, 1, "き"],
          [13, 1, "こう"]
        ],
        "w": [
          [0, 4],
          [4, 3, "だ"],
          [7, 2],
          [9, 1, "が#2028930@*"],
          [10, 2, "無事@2"],
          [12, 2],
          [14, 2, "為る|する"]
        ]
      },
      {
        "j": "たった今漁船が陸を離れた。",
        "en": "A fishing boat put off just now.",
        "r": [
          [3, 1, "いま"],
          [4, 1, "ぎょ"],
          [5, 1, "せん"],
          [7, 1, "りく"],
          [9, 1, "はな"]
        ],
        "w": [
          [0, 4],
          [4, 2],
          [6, 1, "が#2028930@*"],
          [7, 1, "陸|りく"],
          [8, 1, "を@5"],
          [9, 3, "離れる"]
        ]
      }
    ]
  },
  {
    "id": "走行",
    "w": "走行",
    "r": "そうこう",
    "en": [
      "running (of a car, bus, etc.)",
      "traveling",
      "travelling"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "そう",
        "ソウ"
      ],
      [
        1,
        "こう",
        "コウ"
      ]
    ],
    "mis": [
      "そういき",
      "そうごう",
      "そうい",
      "はしこう",
      "そうおこな",
      "そうおこ",
      "ぞうこう",
      "そうゆ"
    ],
    "sp": [
      "逃走",
      "敗走",
      "尾行",
      "犯行",
      "快走",
      "行為",
      "行人",
      "通行",
      "走者",
      "非行",
      "敢行",
      "代行",
      "行書",
      "蛇行",
      "現行",
      "銀行"
    ],
    "ex": [
      {
        "j": "走行マイルで料金が加算されますか。",
        "en": "Is there a mileage charge?",
        "r": [
          [0, 1, "そう"],
          [1, 1, "こう"],
          [6, 1, "りょう"],
          [7, 1, "きん"],
          [9, 1, "か"],
          [10, 1, "さん"]
        ],
        "w": [
          [0, 2],
          [2, 3, "哩"],
          [5, 1, "で#2028980@*"],
          [6, 2],
          [8, 1, "が#2028930@*"],
          [9, 2],
          [11, 4, "為れる"],
          [15, 1, "か@*"]
        ]
      },
      {
        "j": "二輪車が倒れずに走行するのには前輪が大きな役割を演じています。",
        "en": "The front wheel plays an important role in two-wheeled vehicles moving without falling over.",
        "r": [
          [0, 1, "に"],
          [1, 1, "りん"],
          [2, 1, "しゃ"],
          [4, 1, "たお"],
          [8, 1, "そう"],
          [9, 1, "こう"],
          [15, 1, "ぜん"],
          [16, 1, "りん"],
          [18, 1, "おお"],
          [21, 1, "やく"],
          [22, 1, "わり"],
          [24, 1, "えん"]
        ],
        "w": [
          [0, 3],
          [3, 1, "が#2028930@*"],
          [4, 3, "倒れる"],
          [7, 1, "に@*"],
          [8, 2],
          [10, 2, "為る|する"],
          [12, 2, "のに@*"],
          [14, 1, "は@*"],
          [15, 2, "前輪|ぜんりん"],
          [17, 1, "が#2028930@*"],
          [18, 3],
          [21, 9, "役割を演じる"]
        ]
      },
      {
        "j": "走行中の箱乗りはご遠慮下さい！",
        "en": "Please don't lean out of the window when we're moving.",
        "r": [
          [0, 1, "そう"],
          [1, 1, "こう"],
          [2, 1, "ちゅう"],
          [4, 1, "はこ"],
          [5, 1, "の"]
        ],
        "w": [
          [0, 2],
          [2, 1, "中|ちゅう@3"],
          [3, 1, "之|の"],
          [4, 3],
          [7, 1, "は@*"],
          [8, 6, "ご遠慮ください"]
        ]
      }
    ]
  },
  {
    "id": "操業",
    "w": "操業",
    "r": "そうぎょう",
    "en": [
      "operation",
      "work"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "そう",
        "ソウ"
      ],
      [
        1,
        "ぎょう",
        "ギョウ"
      ]
    ],
    "mis": [
      "さんぎょう",
      "そうごう",
      "そうわざ",
      "みさおぎょう",
      "あやつぎょう",
      "ぞうぎょう"
    ],
    "sp": [
      "創業",
      "休業",
      "偉業",
      "企業",
      "業績",
      "仕業",
      "乳業",
      "興業",
      "情操",
      "失業",
      "開業",
      "節操",
      "専業",
      "画業",
      "家業",
      "体操"
    ],
    "ex": [
      {
        "j": "その工場は来月から操業を中止する。",
        "en": "The factory will cease operations next month.",
        "r": [
          [2, 1, "こう"],
          [3, 1, "じょう"],
          [5, 1, "らい"],
          [6, 1, "げつ"],
          [9, 1, "そう"],
          [10, 1, "ぎょう"],
          [12, 1, "ちゅう"],
          [13, 1, "し"]
        ],
        "w": [
          [0, 2, "其の|その"],
          [2, 2, "工場|こうじょう"],
          [4, 1, "は@*"],
          [5, 2],
          [7, 2],
          [9, 2],
          [11, 1, "を@*"],
          [12, 2],
          [14, 2, "為る|する"]
        ]
      },
      {
        "j": "我々の工場はフル操業している。",
        "en": "Our factories are working at full capacity.",
        "r": [
          [0, 2, "われわれ"],
          [3, 1, "こう"],
          [4, 1, "じょう"],
          [8, 1, "そう"],
          [9, 1, "ぎょう"]
        ],
        "w": [
          [0, 2],
          [3, 2, "工場|こうじょう"],
          [5, 1, "は@*"],
          [6, 2],
          [8, 2],
          [10, 4, "為る|する"]
        ]
      },
      {
        "j": "全工場が限界操業をしています。",
        "en": "All of the factories are nearing capacity.",
        "r": [
          [0, 1, "ぜん"],
          [1, 1, "こう"],
          [2, 1, "じょう"],
          [4, 1, "げん"],
          [5, 1, "かい"],
          [6, 1, "そう"],
          [7, 1, "ぎょう"]
        ],
        "w": [
          [0, 1],
          [1, 2, "工場|こうじょう"],
          [3, 1, "が#2028930@*"],
          [4, 2],
          [6, 2],
          [8, 1, "を@*"],
          [9, 5, "為る|する"]
        ]
      }
    ]
  },
  {
    "id": "交通事故",
    "w": "交通事故",
    "r": "こうつうじこ",
    "en": [
      "traffic accident"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "こう",
        "コウ"
      ],
      [
        1,
        "つう",
        "ツウ"
      ],
      [
        2,
        "じ",
        "ジ"
      ],
      [
        3,
        "こ",
        "コ"
      ]
    ],
    "mis": [
      "かつうじこ",
      "まじつうじこ",
      "こうつじこ",
      "こうかよじこ",
      "こうとおじこ",
      "こうつうつかこ",
      "こうつうじもと",
      "こうどおじこ"
    ],
    "sp": [
      "禁止事項",
      "百科事典",
      "刑事責任",
      "同時通訳",
      "基軸通貨",
      "既成事実",
      "外交辞令",
      "事務次官",
      "団体交渉",
      "事件記者",
      "人事異動",
      "事務取扱",
      "事務当局",
      "交感神経",
      "相互交流",
      "商事会社"
    ],
    "ex": [
      {
        "j": "劇場へ行く途中、私は交通事故を見た。",
        "en": "On my way to the theater I saw a traffic accident.",
        "r": [
          [0, 1, "げき"],
          [1, 1, "じょう"],
          [3, 1, "い"],
          [5, 1, "と"],
          [6, 1, "ちゅう"],
          [8, 1, "わたし"],
          [10, 1, "こう"],
          [11, 1, "つう"],
          [12, 1, "じ"],
          [13, 1, "こ"],
          [15, 1, "み"]
        ],
        "w": [
          [0, 2],
          [2, 1],
          [3, 2, "行く|いく"],
          [5, 2],
          [8, 1, "私|わたし"],
          [9, 1, "は@*"],
          [10, 4],
          [14, 1, "を@*"],
          [15, 2, "見る"]
        ]
      },
      {
        "j": "交通事故の数は増加しつつあるようだ。",
        "en": "The number of traffic accidents seems to be on the increase.",
        "r": [
          [0, 1, "こう"],
          [1, 1, "つう"],
          [2, 1, "じ"],
          [3, 1, "こ"],
          [5, 1, "かず"],
          [7, 1, "ぞう"],
          [8, 1, "か"]
        ],
        "w": [
          [0, 4],
          [5, 1, "数|かず"],
          [6, 1, "は@*"],
          [7, 2],
          [9, 1, "為る|する"],
          [10, 4],
          [14, 3, "様だ"]
        ]
      },
      {
        "j": "交通事故で足を折ってしまったんですよ。",
        "en": "I broke my leg in a traffic accident.",
        "r": [
          [0, 1, "こう"],
          [1, 1, "つう"],
          [2, 1, "じ"],
          [3, 1, "こ"],
          [5, 1, "あし"],
          [7, 1, "お"]
        ],
        "w": [
          [0, 4],
          [4, 1, "で#2028980@*"],
          [5, 1, "足|あし@2"],
          [6, 1, "を@*"],
          [7, 3, "折る"],
          [10, 4, "仕舞う"],
          [14, 3, "のです"],
          [17, 1]
        ]
      }
    ]
  },
  {
    "id": "下り",
    "w": "下り",
    "r": "くだり",
    "en": [
      "down-train",
      "train heading toward the ending point of its…",
      "down-slope",
      "downward going",
      "downstream",
      "downhill"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "sn": [
      2,
      2,
      2
    ],
    "ruby": [
      [
        0,
        "くだ"
      ]
    ],
    "mis": [
      "しもり",
      "げり",
      "おり",
      "ぐだり",
      "したり",
      "さり",
      "もとり",
      "かり"
    ],
    "sp": [
      "碎り",
      "拉り",
      "砕り",
      "管り",
      "祉り",
      "蜜り",
      "湊り",
      "旺り",
      "膨り",
      "役り",
      "継り",
      "菱り",
      "丁り",
      "屯り",
      "囚り",
      "甥り"
    ],
    "ex": [
      {
        "j": "午後から天気は下り坂になります。",
        "en": "The weather is supposed to go downhill starting this afternoon.",
        "r": [
          [0, 1, "ご"],
          [1, 1, "ご"],
          [4, 1, "てん"],
          [5, 1, "き"],
          [7, 1, "くだ"],
          [9, 1, "ざか"]
        ],
        "w": [
          [0, 2],
          [2, 2, "から@*"],
          [4, 2],
          [6, 1, "は@*"],
          [7, 3]
        ]
      },
      {
        "j": "道が急な下り坂になっている。",
        "en": "The road declines sharply.",
        "r": [
          [0, 1, "みち"],
          [2, 1, "きゅう"],
          [4, 1, "くだ"],
          [6, 1, "ざか"]
        ],
        "w": [
          [0, 1, "道|みち"],
          [1, 1, "が#2028930@*"],
          [2, 2, "急"],
          [4, 3]
        ]
      },
      {
        "j": "女は３０を過ぎてからはずっと下り坂だ。",
        "en": "For a girl, after 30, it's downhill all the way.",
        "r": [
          [0, 1, "おんな"],
          [5, 1, "す"],
          [14, 1, "くだ"],
          [16, 1, "ざか"]
        ],
        "w": [
          [0, 1, "女|おんな"],
          [1, 1, "は@*"],
          [4, 1, "を@5"],
          [5, 3, "過ぎる|すぎる"],
          [8, 2, "から@6"],
          [10, 1, "は@*"],
          [11, 3],
          [14, 3],
          [17, 1]
        ]
      }
    ]
  },
  {
    "id": "車内",
    "w": "車内",
    "r": "しゃない",
    "en": [
      "inside a car (train, bus, etc.)"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "しゃ",
        "シャ"
      ],
      [
        1,
        "ない",
        "ナイ"
      ]
    ],
    "mis": [
      "しゃうち",
      "しゃだい",
      "くるまない",
      "じゃない"
    ],
    "sp": [
      "馬車",
      "坑内",
      "構内",
      "内部",
      "内奏",
      "室内",
      "学内",
      "内線",
      "屋内",
      "戦車",
      "部内",
      "内堀",
      "区内",
      "内局",
      "内国",
      "飛車"
    ],
    "ex": [
      {
        "j": "氷のような冷気が車内に忍び込んできた。",
        "en": "An icy chill stole into the car.",
        "r": [
          [0, 1, "こおり"],
          [5, 1, "れい"],
          [6, 1, "き"],
          [8, 1, "しゃ"],
          [9, 1, "ない"],
          [11, 1, "しの"],
          [13, 1, "こ"]
        ],
        "w": [
          [0, 1, "氷|こおり"],
          [1, 1, "之|の"],
          [2, 3, "様な"],
          [5, 2],
          [7, 1, "が#2028930@*"],
          [8, 2],
          [10, 1, "に@*"],
          [11, 5, "忍び込む"],
          [16, 2, "来る|くる"]
        ]
      },
      {
        "j": "車内の彼の周りでは、ほかの乗客たちが前後左右に揺れていた。",
        "en": "Around him in the carriage, the other passengers rocked and swayed.",
        "r": [
          [0, 1, "しゃ"],
          [1, 1, "ない"],
          [3, 1, "かれ"],
          [5, 1, "まわ"],
          [13, 1, "じょう"],
          [14, 1, "きゃく"],
          [18, 1, "ぜん"],
          [19, 1, "ご"],
          [20, 1, "さ"],
          [21, 1, "ゆう"],
          [23, 1, "ゆ"]
        ],
        "w": [
          [0, 2],
          [3, 1, "彼|かれ"],
          [5, 2, "周り@2"],
          [7, 1, "で#2028980@*"],
          [8, 1, "は@*"],
          [10, 3, "他|ほか@2"],
          [13, 2, "乗客|じょうきゃく"],
          [15, 2, "達"],
          [17, 1, "が#2028930@*"],
          [18, 4],
          [22, 1, "に@*"],
          [23, 5, "揺れる"]
        ]
      }
    ]
  },
  {
    "id": "船舶",
    "w": "船舶",
    "r": "せんぱく",
    "en": [
      "vessel",
      "ship",
      "shipping"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "ruby": [
      [
        0,
        "せん",
        "セン"
      ],
      [
        1,
        "ぱく",
        "ハク"
      ]
    ],
    "mis": [
      "ふねぱく",
      "ふなぱく",
      "ぜんぱく",
      "せんはく",
      "せんぱっ"
    ],
    "sp": [
      "造船",
      "船橋",
      "船室",
      "船主",
      "配船",
      "船員",
      "船体",
      "船出",
      "船首",
      "風船",
      "汽船",
      "船団",
      "船旅",
      "黒船",
      "船尾",
      "漁船"
    ],
    "ex": [
      {
        "j": "懐中電灯は、船舶同士とか、街の通りを走っている自動車に向けて合図を伝える。",
        "en": "Flashing lights transmit messages between ships and to motorists along city streets.",
        "r": [
          [0, 1, "かい"],
          [1, 1, "ちゅう"],
          [2, 1, "でん"],
          [3, 1, "とう"],
          [6, 1, "せん"],
          [7, 1, "ぱく"],
          [8, 1, "どう"],
          [9, 1, "し"],
          [13, 1, "まち"],
          [15, 1, "とお"],
          [18, 1, "はし"],
          [23, 1, "じ"],
          [24, 1, "どう"],
          [25, 1, "しゃ"],
          [27, 1, "む"],
          [30, 1, "あい"],
          [31, 1, "ず"],
          [33, 1, "つた"]
        ],
        "w": [
          [0, 4],
          [4, 1, "は@*"],
          [6, 2],
          [8, 2, "同士|どうし"],
          [10, 2],
          [13, 1, "街|まち"],
          [15, 2, "通り|とおり"],
          [17, 1, "を@*"],
          [18, 5, "走る"],
          [23, 3],
          [26, 1, "に@*"],
          [27, 3, "向ける"],
          [30, 2],
          [32, 1, "を@*"],
          [33, 3]
        ]
      }
    ]
  },
  {
    "id": "旅客",
    "w": "旅客",
    "r": "りょかく",
    "en": [
      "passenger",
      "traveller",
      "traveler",
      "tourist"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "sn": [
      1,
      3
    ],
    "ruby": [
      [
        0,
        "りょ",
        "リョ"
      ],
      [
        1,
        "かく",
        "カク"
      ]
    ],
    "mis": [
      "りょがっ",
      "たびかく",
      "りょかっ",
      "りょがく"
    ],
    "sp": [
      "旅程",
      "顧客",
      "賓客",
      "客演",
      "客席",
      "客用",
      "主客",
      "浴客",
      "旅館",
      "船旅",
      "旅費",
      "客間",
      "客観",
      "旅先",
      "旅行",
      "旅情"
    ],
    "ex": [
      {
        "j": "覆面をした男達が旅客を襲って金を奪った。",
        "en": "Masked men held up the passengers and robbed them of their money.",
        "r": [
          [0, 1, "ふく"],
          [1, 1, "めん"],
          [5, 1, "おとこ"],
          [6, 1, "たち"],
          [8, 1, "りょ"],
          [9, 1, "かく"],
          [11, 1, "おそ"],
          [14, 1, "かね"],
          [16, 1, "うば"]
        ],
        "w": [
          [0, 2],
          [2, 1, "を@*"],
          [3, 2, "為る|する"],
          [5, 1, "男|おとこ"],
          [6, 1],
          [7, 1, "が#2028930@*"],
          [8, 2, "旅客|りょかく"],
          [10, 1, "を@*"],
          [11, 3, "襲う"],
          [14, 1, "金|かね"],
          [15, 1, "を@*"],
          [16, 3, "奪う"]
        ]
      },
      {
        "j": "旅客は歩道橋を使って路線を渡ってください。",
        "en": "Passengers must take the footbridge to cross the railroad tracks.",
        "r": [
          [0, 1, "りょ"],
          [1, 1, "かく"],
          [3, 1, "ほ"],
          [4, 1, "どう"],
          [5, 1, "きょう"],
          [7, 1, "つか"],
          [10, 1, "ろ"],
          [11, 1, "せん"],
          [13, 1, "わた"]
        ],
        "w": [
          [0, 2, "旅客|りょかく"],
          [2, 1, "は@*"],
          [3, 3],
          [6, 1, "を@*"],
          [7, 3, "使う"],
          [10, 2],
          [12, 1, "を@3"],
          [13, 3, "渡る"],
          [16, 4, "下さい"]
        ]
      },
      {
        "j": "彼らは定期旅客船に乗り込んだ。",
        "en": "They went on board a liner.",
        "r": [
          [0, 1, "かれ"],
          [3, 1, "てい"],
          [4, 1, "き"],
          [5, 1, "りょ"],
          [6, 1, "かく"],
          [7, 1, "せん"],
          [9, 1, "の"],
          [11, 1, "こ"]
        ],
        "w": [
          [0, 2, "彼ら|かれら"],
          [2, 1, "は@*"],
          [3, 2],
          [5, 3, "旅客船|りょかくせん"],
          [8, 1, "に@*"],
          [9, 5, "乗り込む"]
        ]
      }
    ]
  },
  {
    "id": "船長",
    "w": "船長",
    "r": "せんちょう",
    "en": [
      "ship's captain",
      "skipper",
      "length of a ship",
      "boatman"
    ],
    "pos": "n",
    "th": "2.4",
    "lv": "h",
    "sn": [
      2,
      1,
      1
    ],
    "ruby": [
      [
        0,
        "せん",
        "セン"
      ],
      [
        1,
        "ちょう",
        "チョウ"
      ]
    ],
    "mis": [
      "せんぢょう",
      "ふなちょう",
      "せんなが",
      "ふねちょう",
      "ぜんちょう",
      "せんおさ"
    ],
    "sp": [
      "船体",
      "酋長",
      "消長",
      "長柄",
      "長兄",
      "長命",
      "船出",
      "区長",
      "長女",
      "管長",
      "最長",
      "波長",
      "長崎",
      "長話",
      "村長",
      "長久"
    ],
    "ex": [
      {
        "j": "それで船長は彼の世話をしました。",
        "en": "So the captain took care of him.",
        "r": [
          [3, 1, "せん"],
          [4, 1, "ちょう"],
          [6, 1, "かれ"],
          [8, 1, "せ"],
          [9, 1, "わ"]
        ],
        "w": [
          [0, 3, "其れで"],
          [3, 2, "船長|せんちょう"],
          [5, 1, "は@*"],
          [6, 1, "彼|かれ"],
          [8, 7, "世話をする"]
        ]
      },
      {
        "j": "私は船長に命を託さなければならなかった。",
        "en": "I had to trust the captain with my life.",
        "r": [
          [0, 1, "わたし"],
          [2, 1, "せん"],
          [3, 1, "ちょう"],
          [5, 1, "いのち"],
          [7, 1, "たく"]
        ],
        "w": [
          [0, 1, "私|わたし"],
          [1, 1, "は@*"],
          [2, 2, "船長|せんちょう"],
          [4, 1, "に@*"],
          [5, 1, "命|いのち"],
          [6, 1, "を@*"],
          [7, 2, "託す"],
          [9, 10, "ねばならない"]
        ]
      },
      {
        "j": "船長というのは、船と乗組員に対して責任がある。",
        "en": "Captains have responsibility for ship and crew.",
        "r": [
          [0, 1, "せん"],
          [1, 1, "ちょう"],
          [8, 1, "ふね"],
          [10, 1, "の"],
          [11, 1, "りくみ"],
          [12, 1, "いん"],
          [14, 1, "たい"],
          [17, 1, "せき"],
          [18, 1, "にん"]
        ],
        "w": [
          [0, 2, "船長|せんちょう"],
          [2, 5, "と言うのは"],
          [8, 1, "船|ふね"],
          [9, 1, "と@*"],
          [10, 3],
          [13, 4],
          [17, 2],
          [19, 1, "が#2028930"],
          [20, 2, "有る"]
        ]
      }
    ]
  }
];
