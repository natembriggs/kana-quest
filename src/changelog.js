...
export const CHANGELOG = [
  {
    date: '2026-09-03',
    changes: [
      'New: "Mark as known" — already read hiragana, or know a kanji grade from outside the app? Open View set overview (or tap the new row under Test unlearned) and tick what you already know, or use "Select all not started" to grab a whole unit in one go. For Reading, Definition and Meaning — where a glance at the character is enough to judge — it marks them mastered straight away. For Yomi, Writing and Recall, where a glance can\'t tell you everything (you might know some of a kanji\'s readings without noticing you don\'t know all of them, or recognise a character without really being able to write it), the default is a softer "I think I know this": each one comes back for a quick double-check spread over the following few weeks, not all on the same day, so a big batch never turns into one huge review pile-up. An "I\'m sure" option sits right beside it if you really are certain. Either way, nothing is permanently skipped — everything still comes back for review.',
