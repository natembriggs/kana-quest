...
# Meanings KANJIDIC lists too far down its own ordering (not ranked by
# importance) to survive the [:4] cutoff in parse_kanjidic() below, but that
# this app's own curated example words show are actually central to how the
# kanji is used — e.g. 調's meanings used to read "tune, tone, meter, key
# (music)" with no mention of "investigate", despite 調べ/調査 ("investigation")
# being its own example words. Found by cross-checking every kanji whose
# stored list hit the cap against KANJIDIC's real, untruncated list — see
# kanji-meanings-audit.md for the full audit and evidence per kanji. Applied
# as an EXTRA slot in parse_kanjidic(), not a replacement, so this never also
# relitigates what the existing four should be.
MEANING_INCLUDE = {
    "中": ["center"],
    "見": ["opinion"],
...
def parse_kanjidic():
    ...
    for block in re.findall(r"<character>.*?</character>", text, re.S):
        ...
        kept_meanings = [m for m in meanings if not RADICAL_MEANING.search(m)][:4]
        for extra in MEANING_INCLUDE.get(literal, []):
            if extra not in kept_meanings:
                kept_meanings.append(extra)
        out[literal] = {
            "on": re.findall(r'<reading r_type="ja_on">(.*?)</reading>', block),
            "kun": re.findall(r'<reading r_type="ja_kun">(.*?)</reading>', block),
            "meanings": kept_meanings,
